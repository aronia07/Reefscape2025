...
public class LEDSubsystem extends SubsystemBase {
    ...
    //However, multiple LED strips can be connected in series and controlled from the single driver.

    //Setup
    private static final int kPort = 9;  //PWM Port
    private static final int kLength = 120; //LED strip length [# of LEDs]
    private static final Distance kLedSpacing = Meters.of(1 / 120.0);   //LED strip LEDs density - [... LEDs per meter]
    ...
    public LEDSubsystem() {
      ...
      //m_left = m_ledbuffer.createView(0, kLength/2 - 1);  //Left side of the LED strip
      //m_right = m_ledbuffer.createView(kLength/2, kLength - 1).reversed();  //Right side of the LED strip
     ...
     * @implNote Recomended pattern input (rainbow): private final LEDPattern m_rainbow = LEDPattern.rainbow(255, 128);
     */
...
// physical LED strip on the robot.
    ...
    //Animated Rainbow
      ...
      // Pattern that scrolls the rainbow pattern across the LED strip, moving at a speed
